...
class _ViewNotePageState extends State<ViewNotePage> {
  ...
  void _createController() {
    ...
    try {
      if (data.isEmpty) {
        ...
        document = Document.fromJson(List<dynamic>.from(data));
      }
    } catch (_) {
      document = Document();
    }
  ...
  Future<void> _editNote() async {
    final result = await Navigator.of(context).push<NoteModel>(
      MaterialPageRoute(
        builder: (context) {
          return CreateNotePage(note: _note);
        },
      ),
    );
  ...
  void _shareNote() {
    NoteShareSheet.show(context, _note);
  }
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              tooltip: 'Share',
              icon: const Icon(Icons.share_outlined, color: Color(0xFF5B5FEF)),
            ),

            IconButton(
              onPressed: _editNote,
              tooltip: 'Edit',
              icon: const Icon(Icons.edit_outlined, color: Color(0xFF5B5FEF)),
            ),
          ],
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              if (note.pages.length > 1) ...[
                const SizedBox(height: 20),
                ViewNotePagesPreview(note: note),
              ],

              if (note.imagePaths.isNotEmpty) ...[
                const SizedBox(height: 24),
                ViewNoteImages(note: note),
              ],

              if (note.pdfs.isNotEmpty) ...[
                const SizedBox(height: 24),
                ViewNotePdfs(note: note),
              ],
            ],
          ),
        ),
